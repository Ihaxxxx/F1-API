...
export default function PositionGraph({
    ...
    <Card className="w-full bg-black border border-white rounded-2xl shadow-md mt-4">
      <CardHeader>
        <CardTitle className="capitalize text-white font-f1bold text-xl md:text-2xl">
          ...
          {driverAndRoundDetail.year} - Position Graph
        </CardTitle>
      </CardHeader>
        ...
        <ResponsiveContainer width="100%" height={300}>
          ...
          >
            ...
            <Line
              ...
              strokeWidth={2}
              dot={({ cx, cy, payload }) => {
                const color = payload.pitStop ? "#FFD700" : "#00C49F";
                return (
                  <circle
                    key={`lap-dot-${payload.lap}`}
                    cx={cx}
                    cy={cy}
                    r={4}
                    fill={color}
